...
export const SeriesComments = ({ movieId }) => {
	...
	const handleCommentSubmit = async (e) => {
		...
		try {
			// Envia o comentário
			await api.post("/series/comments", {
				content,
				id_user: user.id,
				id_serie_ref: movieId
			});
	...
	const fetchComments = async () => {
		try {
			const response = await api.get(`/series/comments?id_serie_ref=${movieId}`);
